...
# ---------------- Update Donor Window ---------------- #
def open_update_window(parent, donor_id):
    ...
    def save_update():
        values = (
            txt_donor_name.get(), txt_age.get(), "Male" if gender_var.get()==1 else "Female",
            bg_var.get(), txt_phone.get(), txt_email.get(), txt_address.get(),
            txt_last_donation.get(), txt_next_donation.get(), txt_registration.get(),
            txt_donor_id.get()
        )
        if all(values[:-1]):  # Check all except donor_id
...
def open_search_window(parent):
    ...
    def search_donor():
        ...
        if get_donor_by_id(donor_id_int):
            win.destroy()
            open_update_window(parent, donor_id_int)
        else:
            messagebox.showerror("Error", f"No donor found with ID {donor_id}")
